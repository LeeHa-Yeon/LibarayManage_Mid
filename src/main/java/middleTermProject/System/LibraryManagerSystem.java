    ...
    public void addBook() {
        ...
        try {
            ...
            if(idList.contains(Integer.parseInt(newBook[1]))) {
                ...
                bw_books.write(String.format("%d/%d/%s/%s/%s/%s/%d/%s/%b", Integer.parseInt(newBook[0]), Integer.parseInt(newBook[1]), newBook[2], newBook[3], newBook[4], newBook[5], 1,"대여가능", true));
                bw_book.write(String.format("%d/%d/%s/%s/%s/%s/%d/%s/%b", Integer.parseInt(newBook[0]), Integer.parseInt(newBook[1]), newBook[2], newBook[3], newBook[4], newBook[5], 1, "대여가능", true));
